...
deploy-dev-unbuild:     
	...
	ssh ${DEV_HOST} -p ${SSH_PORT} "IMG=${IMAGE} TAG=${RELEASE_VERSION} DIR=${DEV_BASE}/${DEPLOY_DIR} MODE=debug docker stack deploy -c ${DEPLOY_DIR}/docker/run-dev.yml erik --with-registry-auth"
